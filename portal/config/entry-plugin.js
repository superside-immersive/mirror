...
delete fallbackScene.history
delete fallbackScene.historyVersion
...
const loadSceneGraph = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error('Scene endpoint unavailable')
    }

    const nextScene = await response.json()

    delete nextScene.history
    delete nextScene.historyVersion
